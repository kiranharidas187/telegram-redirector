...
- **`app.py`** — a local web portal (FastAPI + vanilla JS, no build step) for Telegram login, picking channels, assigning voices, and watching a live activity feed from a browser. This is the primary/day-to-day path. Binds to `127.0.0.1` only, no portal-level auth (the OS account is the trust boundary). Config (channels, per-channel voice/mute, speech rate, volume) lives in `config.json`, a plain gitignored JSON file — no database.
